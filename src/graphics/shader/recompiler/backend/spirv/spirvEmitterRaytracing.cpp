...
void DefineBvhIntersect(EmitterState& state) {
	...
	const auto box_label   = state.builder.AllocateId();
	const auto other_label = state.builder.AllocateId();
	const auto merge_label = state.builder.AllocateId();
	const auto entry_exit  = state.builder.AllocateId();
	state.builder.AddFunction({OpSelectionMerge, merge_label, SelectionControlNone});
	state.builder.AddFunction({OpBranchConditional, is_box32, box_label, other_label});

	EmitLabel(state, box_label);
	// A 128-byte node spans two 64-byte units, which may sit in different caching pages, so
	// each half is resolved separately. Offsets then stay inside the resolved page.
	...
	const auto real_dword = [&](uint32_t index) {
		return Op1(state, OpBitcast, f32, dword(index));
	};
	...
	for (uint32_t slot = 0; slot < 4u; slot++) {
		...
		const std::array<uint32_t, 3> box_min {real_dword(first), real_dword(first + 1),
		                                       real_dword(first + 2)};
		const std::array<uint32_t, 3> box_max {real_dword(first + 3), real_dword(first + 4),
		                                       real_dword(first + 5)};
		const auto slab = Slab(state, ray, box_min, box_max, grow_scale);
		const auto live = Op2(state, OpLogicalAnd, bool_type, slab.hit, mapped);
		child[slot]     = Op3(state, OpSelect, u32, live, dword(slot), invalid);
		key[slot]       = slab.near_distance;
	}
	SortChildren(state, child, key, sort_enabled);

	const auto box_result = state.builder.AllocateId();
	state.builder.AddFunction({OpCompositeConstruct, result_type, box_result, child[0], child[1],
	                           child[2], child[3]});
	state.builder.AddFunction({OpBranch, entry_exit});
	EmitLabel(state, entry_exit);
	...
	const auto result = state.builder.AllocateId();
	state.builder.AddFunction({OpPhi, result_type, result, box_result, entry_exit, all_missed,
	                           other_label});
